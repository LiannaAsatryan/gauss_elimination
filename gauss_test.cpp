...
//declarations
bool match(std::fstream&, std::string);
void test();
...
//this function performs the following operations for every matrix in the input file
...
// 4)calls gauss function which stores the solutions in the <x> array
// 5)calls the <set_answer> function which turns our answer to a string
...
void testing(std::fstream &i_file, std::fstream &o_file, std::fstream &r_file, std::fstream &g_file) 
{
...
void test()
{
		...
		testing(i_file, o_file, r_file, g_file);
        }
        r_file.close();
        i_file.close();
        o_file.close();
